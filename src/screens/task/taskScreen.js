...
const TaskScreen = () => {
  ...
  // Auth Store - Get user and permission helpers
  ...
  const [selectedView, setSelectedView] = useState("my"); // Default to "my"
  ...
  // Get current task list based on view and role
  const getCurrentTasks = () => {
    if (selectedView === "my") {
      // My Tasks - Always accessible
      return myTasks || [];
    }
    
    // All Tasks view - Only for HR Manager and Team Lead
    if (user?.role === "hr_manager" || user?.role === "team_lead") {
      return getFilteredTasks();
    }
    
    // Fallback to my tasks if no permission
    return myTasks || [];
  };
